...
class CommentIntegrationTest {
  ...
  void setUp() {
    var user1 =
        userRepository.save(
            new UserJpaEntity(
                "author@test.com", "author", passwordEncoder.encode("pass"), null, null));
    token1 = jwtTokenProvider.generateToken(user1.getId());

    var user2 =
        userRepository.save(
            new UserJpaEntity(
                "other@test.com", "other", passwordEncoder.encode("pass"), null, null));
  ...
  void addComment_returns200() throws Exception {
    String body =
        objectMapper.writeValueAsString(Map.of("comment", Map.of("body", "Nice article!")));

    mockMvc
        .perform(
            post("/api/articles/{slug}/comments", articleSlug)
  ...
  void addComment_unauthenticated_returns401() throws Exception {
    String body = objectMapper.writeValueAsString(Map.of("comment", Map.of("body", "text")));
  ...
  void listComments_returnsEmptyInitially() throws Exception {
    mockMvc
        .perform(get("/api/articles/{slug}/comments", articleSlug))
        .andExpect(status().isOk())
  ...
  void listComments_afterAdding_returnsComments() throws Exception {
    // Add a comment
    String body = objectMapper.writeValueAsString(Map.of("comment", Map.of("body", "Comment 1")));
    mockMvc
        .perform(
            post("/api/articles/{slug}/comments", articleSlug)
                .header("Authorization", "Token " + token1)
  ...
  void deleteComment_byAuthor_returns200() throws Exception {
    // Add comment
    String body = objectMapper.writeValueAsString(Map.of("comment", Map.of("body", "To delete")));
    MvcResult result =
        mockMvc
            .perform(
                post("/api/articles/{slug}/comments", articleSlug)
    ...
    mockMvc
        .perform(
            delete("/api/articles/{slug}/comments/{id}", articleSlug, commentId)
                .header("Authorization", "Token " + token1))
        .andExpect(status().isOk());
  ...
  void deleteComment_byNonAuthor_returns403() throws Exception {
    ...
    String body = objectMapper.writeValueAsString(Map.of("comment", Map.of("body", "Protected")));
    MvcResult result =
        mockMvc
            .perform(
                post("/api/articles/{slug}/comments", articleSlug)
  ...
  void deleteComment_unauthenticated_returns401() throws Exception {
    mockMvc
        .perform(delete("/api/articles/{slug}/comments/1", articleSlug))
        .andExpect(status().isUnauthorized());
  }
  ...
  void addComment_articleNotFound_returns404() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/articles/{slug}/comments", "nonexistent")
                .header("Authorization", "Token " + token1)
                .contentType(MediaType.APPLICATION_JSON)
                .content(body))
